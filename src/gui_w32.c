...
process_message(void)
{
	...
	if (special_keys[i].key_sym == 0)
	{
		   ...
		   )
		{
		    ...
		    // not a dead key or dead key without Ctrl or it is neither
		    // QWERTZ nor AZERTY CTRL+DEAD_KEY - anyway we don't have
		    // an idea for special handling - leave as it is
		    return;
	    }
	    else
	    {
