    ...
    int cRepeat)
{
    ...
    if (ans_file) { fprintf(ans_file, "b simlpl_k: %d, %d; ", ch, modifiers); }
	...
	if (cRepeat & LPARAM_WM_CHAR_IGNORE_CTRL)
	    fprintf(ans_file, "... removing CTRL modifier ...\n");
	if (cRepeat & LPARAM_WM_CHAR_IGNORE_ALT)
	    fprintf(ans_file, "... removing ALT modifier ...\n");
	if (cRepeat & LPARAM_WM_CHAR_IGNORE_SHIFT)
	    fprintf(ans_file, "... removing SHIFT modifier ...\n");
    }
...
outputDeadKey_rePost_Ex(MSG originalMsg, int dead_key2set)
{
    ...
    if (ans_file) { fprintf(ans_file, "outputDeadKey_rePost_Ex(%d)\n", dead_key2set); fflush(ans_file); }
...
process_message(void)
{
	...
	if (ans_file)
	{
	    ...
	    BOOL ret = GetKeyboardLayoutName(
		      //[out] LPSTR pwszKLID
		    name
	    );
	    if (!ret) {
		fprintf(ans_file, "GetKeyboardLayoutNameA failed\n");
	    } else {
		fprintf(ans_file, "GetKeyboardLayoutName: ");
		for (i=0;name[i];++i) {
		    fprintf(ans_file, "%c", (char)name[i]);
		}
		fprintf(ans_file, "\n");
		fflush(ans_file);
	    }
	...
	if (special_keys[i].key_sym == 0)
	{
		...
		else */ if (vk == 0xBD)	// QWERTY for CTRL-'-'
		{
		    string[0] = Ctrl__;
		    add_to_input_buf(string, 1);
		    if (ans_file) { fprintf(ans_file, "....vk=%d=0xBD for ^_ workaround\n", vk); fflush(ans_file); }
		    return;
		} /*
		...
		*/
	    ...
	    if (ans_file) { fprintf(ans_file, "...Not a special key. (big diff to v8_2_4005)\n"); fflush(ans_file); }
